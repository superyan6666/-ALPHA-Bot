...
def train_production_model():
    ...
    log.info(f"Training on rolling window: {train_dates[0].strftime('%Y-%m-%d')} to {train_dates[-1].strftime('%Y-%m-%d')} ({len(train_dates)} days)")
    ...
    log.info("Top 5 Feature Importances (Gain):")
    ...
    if len(dates) > 30:
        ...
        # Rank IC calculation
        def calc_ic(g):
            return g['xgb_score'].corr(g['fwd_ret_real'], method='spearman')
            
        ic_series = val_df.groupby('date').apply(calc_ic)
        mean_ic = ic_series.mean()
        log.info(f"Recent 30-day Mean Rank IC proxy: {mean_ic:.4f}")
        
    os.makedirs('.quantbot_data', exist_ok=True)
    model_path = '.quantbot_data/prod_xgb_model.json'
    meta_path = '.quantbot_data/prod_xgb_meta.json'
    ...
    metadata = {
        ...
        'mean_ic_30d': float(mean_ic) if len(dates) > 30 else None
    }
    
    with open(meta_path, 'w') as f:
        json.dump(metadata, f, indent=4)
        
    log.info("Production model and metadata saved successfully.")
